...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-700 font-sans antialiased flex flex-col justify-between">
      ...
      <div>
        ...
        <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center">
          <div>
            ...
            <p className="text-slate-500 text-xs sm:text-sm leading-relaxed max-w-md mb-8">
                ...
                <div className="bg-slate-50/50 border border-slate-100 rounded-lg p-5 sticky top-24">
                  ...
                  {stack.length === 0 ? (
                    ...
                    <div className="flex flex-col space-y-3">
                      {stack.map((item) => (
                        <div key={item.id} className="flex items-center justify-between bg-white border border-slate-100 p-3 rounded-md shadow-xs">
                          <div className="flex items-center space-x-3">
                            <img src={item.icon} alt={item.name} className="w-5 h-5 object-contain" />
                            <div>
                              <h4 className="text-xs font-semibold text-slate-800 leading-none">{item.name}</h4>
                              <span className="text-[10px] text-slate-400">{item.category}</span>
                            </div>
                          </div>
                          <button
                            onClick={() => handleRemoveFromStack(item.id, item.name)}
                            className="text-slate-400 hover:text-red-500 text-xs p-1 font-bold transition"
                            title="Remove"
                          >
                            ✕
                          </button>
                      ...
                      <button
                        ...
                        className="w-full mt-2 py-2 border border-red-200 text-red-500 rounded-md text-xs font-medium hover:bg-red-50/50 transition"
                      >
                        Remove All
                      </button>
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 mb-8 text-xs">
          <div>
            ...
            <div className="flex space-x-4 text-slate-500 font-medium">
              ...
              <a href="https://linkedin.com" target="_blank" rel="noreferrer" className="hover:text-slate-900 transition">LinkedIn</a>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-slate-400">
              <li><a href="#" className="hover:text-slate-600 transition">Home</a></li>
              <li><a href="#technologies" className="hover:text-slate-600 transition">Technologies</a></li>
              <li><a href="#" className="hover:text-slate-600 transition">Projects</a></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-slate-400">
              <li><a href="#" className="hover:text-slate-600 transition">About</a></li>
              <li><a href="#" className="hover:text-slate-600 transition">Contact</a></li>
              <li><a href="#" className="hover:text-slate-600 transition">Careers</a></li>
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2 text-slate-400">
              <li><a href="#" className="hover:text-slate-600 transition">Privacy Policy</a></li>
              <li><a href="#" className="hover:text-slate-600 transition">Terms of Service</a></li>
            </ul>
          </div>
        </div>
